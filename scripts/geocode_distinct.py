...
def geocode_address(results):
	...
	writer.writerow([company] + ["First", uniencoder(county), uniencoder(city_name), uniencoder(postal_code), uniencoder(address), lat_coords, long_coords])
...
with open(outputfile, 'wb') as fout:
	...
	for company in beneficiaries:
		PLACES_URL = "https://maps.googleapis.com/maps/api/place/textsearch/json?query="+company+" "+country+"&key="+API_KEY
		...
		if result_zero["status"] == "OK" or result_zero["status"] == "ZERO_RESULTS":
			if len(result_zero["results"]) > 0:
				...
				for i in beneficiaries[company]["variations"]:
			 		PLACES_URL_v = "https://maps.googleapis.com/maps/api/place/textsearch/json?query="+i+" "+country+"&key="+API_KEY
					...
					if beneficiaries[company]["variations"].index(i) == len(beneficiaries[company]["variations"]) - 1:
						for i in beneficiaries[company]["variations"]:
							PLACES_URL_v2 = "https://maps.googleapis.com/maps/api/place/textsearch/json?query="+i+"&key="+API_KEY
